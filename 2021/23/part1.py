...
def find(mat, score, depth):
    ...
    if np.array_equal(mat, [['.', '.', '.', '.', '.', '.', '.', '.', '.', '.', '.'],
        ...
        return
    for j in [2, 4, 6, 8]:
        if np.count_nonzero(mat[1:, j] == columnIndexNum[j]) + np.count_nonzero(mat[1:, j] == '.') != 2:
            for i in range(1, 3):
                if mat[i, j] != '.':
                    column = columnIndexLetter[mat[i, j]]
                    if np.count_nonzero(mat[1:, column] == mat[i, j]) + np.count_nonzero(mat[1:, column] == '.') != 2:
                        ...
                        possible, length = isPathFree(mat,i, j, 2 - np.count_nonzero(mat[1:, column] == mat[i, j]), column)
                        if possible:
                            newMat = copy.deepcopy(mat)
                            ii = 2 - np.count_nonzero(mat[1:, column] == mat[i, j])
                            jj = column
                            newMat[ii][jj] = newMat[i][j]
                            newMat[i][j] = '.'
                            find(newMat, score + length * scores[mat[i, j]], depth+1)
